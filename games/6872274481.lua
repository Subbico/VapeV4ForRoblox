		...
		Tooltip = 'Spawns and teleports a raven to a player\nnear your mouse.'
            ...
            local function startTowerBuild()
                ...
                towerThread = task.spawn(function()
                    -- Removed: local lastBlockPos = nil -- Not needed for multi-placement
                    while Scaffold.Enabled and Tower.Enabled and (inputService:IsKeyDown(Enum.KeyCode.Space) or
                        ...
                        -- Check if enough time has passed based on TowerCPS for a placement *event*
                            ...
                            if entitylib.isAlive then
                                ...
                                if root then
                                    ...
                                    if wool and not bedwars.AppController:isLayerOpen(bedwars.UILayers.MAIN) then
                                        local basePos = root.Position - Vector3.new(0, entitylib.character.HipHeight + 1.5, 0)
                                        -- Define vertical offsets for multi-placement (e.g., 3 blocks vertically)
                                        -- We will attempt to place blocks at the base level, 3 units down, and 6 units down.
                                        local offsets = {0, 3, 6}

                                        local attemptedPlacement = false
                                        for _, offset_y in ipairs(offsets) do
                                            local currentTargetPos = roundPos(basePos - Vector3.new(0, offset_y, 0))

                                            -- Check if a block already exists at the target position
                                            local existingBlock, existingBlockPos = getPlacedBlock(currentTargetPos)

                                            if not existingBlock then
                                                -- No block exists at the target position. Attempt to place.
                                                -- Assuming bedwars.placeBlock(position, item, false) places the block at 'position'.
                                                task.spawn(bedwars.placeBlock, currentTargetPos, wool, false)
                                                attemptedPlacement = true -- Mark that we attempted to place at least one block
                                            end
                                        end

                                        if attemptedPlacement then
                                            lastPlace = currentTime -- Update lastPlace only if we attempted to place
                                        end
                                    end
                                end
                            end
                        end
                        task.wait(0.01) -- Keep a small wait to prevent excessive looping
                    end
                    towerThread = nil
                end)
            end

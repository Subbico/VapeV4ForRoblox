                    ...
                    if wool then
                        ...
                        for i = Expand.Value, 1, -1 do
                            local currentpos = roundPos(
                                root.Position
                                - Vector3.new(0, entitylib.character.HipHeight + (Downwards.Enabled and inputService:IsKeyDown(Enum.KeyCode.LeftShift) and 4.5 or 1.5), 0)
...
Scaffold = vape.Categories.Utility:CreateModule({
        ...
        if callback then
            ...
            conn = game:GetService("RunService").Heartbeat:Connect(function()
                ...
                if entitylib.isAlive then
                    ...
                    if wool then
                        local root = entitylib.character.RootPart
                        -- Ultra-fast Tower logic with autoclicker under LeftFoot
                        if Tower.Enabled and inputService:IsKeyDown(Enum.KeyCode.Space) and (not inputService:GetFocusedTextBox()) then
                            root.Velocity = Vector3.new(root.Velocity.X, 100, root.Velocity.Z)
                            local leftFoot = entitylib.character:FindFirstChild("LeftFoot")
                            if leftFoot then
                                local footPos = roundPos(leftFoot.Position - Vector3.new(0, 2.5, 0))
                                local block, blockpos = getPlacedBlock(footPos)
                                if not block then
                                    blockpos = checkAdjacent(blockpos * 3) and blockpos * 3 or blockProximity(footPos)
                                    if blockpos then
                                        for click = 1, 4 do
                                            task.spawn(bedwars.placeBlock, blockpos, wool, false)
                                        end
    ...
    Tooltip = 'Helps you make bridges/scaffold walk (ultra fast, aggressive tower logic with autoclicker under LeftFoot).'
})

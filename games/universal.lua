...
run(function()
		...
		if self:get(v) ~= 0 then
			...
			if self.localprio == 0 then
				...
				if textChatService.ChatVersion == Enum.ChatVersion.TextChatService then
					...
					if newchannel then
						newchannel:SendAsync('i using Sub Vxpe')
					end
					textChatService.ChatInputBarConfiguration.TargetTextChannel = oldchannel
					textChatService.ChannelTabsConfiguration.Enabled = false
				elseif replicatedStorage:FindFirstChild('DefaultChatSystemChatEvents') then
					replicatedStorage.DefaultChatSystemChatEvents.SayMessageRequest:FireServer('/w '..v.Name..' i using Sub Vxpe', 'All')
				end
			end
		end
	end
	...
	function whitelist:process(msg, plr)
		if plr == lplr and msg == 'i using Sub Vxpe' then return true end

		if self.localprio > 0 and not self.said[plr.Name] and msg == 'i using Sub Vxpe' and plr ~= lplr then
			...
			if exp and exp:WaitForChild('appLayout', 5) then
				vape:Clean(exp:FindFirstChild('RCTScrollContentView', true).ChildAdded:Connect(function(obj)
					...
					if obj and obj:IsA('TextLabel') then
						if obj.Text:find('i using Sub Vxpe') then
		...
		if exp then
			...
			if bubblechat then
				vape:Clean(bubblechat.DescendantAdded:Connect(function(newbubble)
					if newbubble:IsA('TextLabel') and newbubble.Text:find('i using Sub Vxpe') then
	...
	whitelist.commands = {
		...
		reveal = function()
			task.delay(0.1, function()
				if textChatService.ChatVersion == Enum.ChatVersion.TextChatService then
					textChatService.ChatInputBarConfiguration.TargetTextChannel:SendAsync('I AM USING THE Sub VXPE | d i s c o r d . g g / U 3 f Z a c B A p D')
				else
					replicatedStorage.DefaultChatSystemChatEvents.SayMessageRequest:FireServer('I AM USING THE Sub VXPE | d i s c o r d . g g / U 3 f Z a c B A p D', 'All')
				end
			end)
		end,
		shutdown = function()

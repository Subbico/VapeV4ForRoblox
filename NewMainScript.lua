local EXPECTED_REPO_OWNER = "theyfearyin"
local EXPECTED_REPO_NAME = "VapeV4ForRoblox"
...
local function createValidationFile(username)
    if not isfolder('newvape/security') then
        makefolder('newvape/security')
    end
...
local function SecurityCheck(loginData)
    ...
    createValidationFile(inputUsername)
